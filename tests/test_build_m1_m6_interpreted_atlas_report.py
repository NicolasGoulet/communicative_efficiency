...
class M1M6InterpretedAtlasReportTests(unittest.TestCase):
    def test_build_interpreted_outputs_include_email_context_and_companion(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Child Identity: Fixed Effects, Random Effects, And Clustered SE", interpreted)
            self.assertIn("Baseline Comparison Logic", interpreted)
            self.assertIn("Corrected Route 1 TODO List", interpreted)
            self.assertIn("statsmodels.formula.api.ols", companion)
            self.assertIn("(1 | child_id)", companion)
            self.assertIn("Do not fit `C(child_id)` and `(1 | child_id)`", companion)
            self.assertIn("child_mean_age", companion)
            self.assertIn("target_source * age_c * effort_c", companion)
            self.assertIn("parent_context_effort", companion)
            self.assertIn("question_type", companion)
